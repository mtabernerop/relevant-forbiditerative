...
import copy_plans
from planner_call import BaseCostOptimalPlannerCall, make_call, get_base_dir
...
def find_plans(args):
    ...
    planner.report_number_of_valid_plans(plan_manager)
    ...
    logging.info("Extending plan with structural symmetries")
    command = planner.get_extend_plans_callstring(task_manager, plan_manager)
    if command is not None:
        ## Adding plans from the just found plan
        time_limit = limits.get_time_limit(None, args.overall_time_limit)
        try:
            _timers["extending_plans"].start()
            make_call(command, time_limit, local_folder, enable_output=enable_planners_output)
            _timers["extending_plans"].stop()
        except subprocess.TimeoutExpired:
            print("\n")
            logging.warning("Timeout expired. End of execution.")
            planner.report_iteration_step(plan_manager, success=False)
    ...
    logging.debug("Task reformulation")
    command = planner.get_reformulation_callstring(task_manager, plan_manager)
    time_limit = limits.get_time_limit(None, args.overall_time_limit)
    try:
        _timers["task_reformulation"].start()
        make_call(command, time_limit, local_folder, enable_output=enable_planners_output)
        _timers["task_reformulation"].stop()
    except subprocess.TimeoutExpired:
        print("\n")
        logging.warning("Timeout expired. End of execution.")
        planner.report_iteration_step(plan_manager, success=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--global-time-limit", help="Global time limit", type=int)
