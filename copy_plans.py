...
def map_back_fast_downward_plan_file(original, copy):
    lines = []
...
# match and rewrite operators in base as in original
def map_fast_downward_plan_file(base, original):
    mapped_text = ''
    counter = 0
    with open(base, 'r') as b, open(original, 'r') as o:
        btext = b.readlines()
        otext = o.readlines()
        for line in btext:
            stripped_line = line.strip()
            if line.startswith(';'):
                mapped_text += line + '\n'
                break
            while stripped_line !=  (otext[counter].strip().split('_')[0] + ')'):
                counter += 1
            mapped_text += otext[counter]

    with open(base, 'w') as b:
        b.writelines(mapped_text)

# remove text order parameters in actions of plan
def map_back_plan_order_parameters(plan):
...
# remove files in directory with given extension
def delete_ext_files(directory, extension):
    files_in_dir = os.listdir(directory)
    filtered_files = [file for file in files_in_dir if file.endswith(extension)]
    count = 0
    for file in filtered_files:
        path_to_file = os.path.join(directory, file)
        os.remove(path_to_file)
        count += 1
    logging.warning(f"Deleting files with extension {extension} within {directory}")
    logging.info(f"Successfully deleted {count} file(s)")
...
# modify cost of plan to value
def modify_cost(plan, value):
    with open(plan, "r") as f:
        lines = f.read()
        lines = lines.split("\n")
        unparsed_lines = []
